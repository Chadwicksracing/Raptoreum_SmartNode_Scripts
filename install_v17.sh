...
function install_bins() {
  ...
  rm -rf temp
  wget $POWCACHE
  sudo mv $HOME/powcache.dat $CONFIG_DIR
}
...
WALLET_TAR=\$(curl -s https://github.com/Raptor3um/raptoreum/releases/tag/1.13.17.0 | jq -r '.assets[] | select(.name|test("'\$VERSION'.")) | .browser_download_url')
COIN_NAME='raptoreum'
COIN_DAEMON='raptoreumd'
COIN_CLI='raptoreum-cli'
COIN_TX='raptoreum-tx'
